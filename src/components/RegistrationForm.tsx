  ...
  if (step === 3 && result) {
    return (
      <div className="flex flex-col items-center text-center space-y-6 py-8">
        ...
        <div>
          <h2 className="text-2xl font-bold text-white mb-2">{result.name} is Live!</h2>
        ...
        <div className="w-full p-4 rounded-xl bg-white/5 border border-white/10 text-left space-y-2">
          <p className="text-xs text-gray-400 uppercase tracking-widest">Your Profile URL</p>
          <p className="text-indigo-400 font-mono text-sm break-all">
            {typeof window !== 'undefined' ? window.location.origin : ''}/{result.slug}
          </p>
        </div>
        <div className="flex flex-col sm:flex-row gap-3 w-full">
          ...
          <button
            onClick={() => { setStep(1); setName(''); setDescription(''); setLogo(''); setLinks([]); setResult(null); }}
